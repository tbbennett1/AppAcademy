...
def toyless_blue_cats
  ...
  LEFT JOIN toys ON toys.id = cattoys.toy_id
  WHERE cats.color = 'Navy Blue' AND toys.id IS NULL;
  SQL
end
...
def cats_like_johnson
  ...
  WHERE cats.breed = (
      SELECT cats.breed
      FROM cats
      WHERE cats.name = 'Johnson' AND cats.color = 'Lavender'
      )
  ORDER BY cats.name ASC;
...
def expensive_tastes
  ...
  execute(<<-SQL)
  SELECT DISTINCT cats.name, toys.name, toys.color
  FROM toys
  JOIN cattoys ON cattoys.toy_id = toys.id
  JOIN cats ON cattoys.cat_id = cats.id
  WHERE toys.id = (
