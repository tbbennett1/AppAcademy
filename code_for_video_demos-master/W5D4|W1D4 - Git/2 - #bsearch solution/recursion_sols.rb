...
def range(min, max)
  ...
  def bsearch(nums, target)
    nums #=> [5,6,7,8,9], [8, 9], [8]
    target #=> 8, 8, 8
    # nil if not found; can't find anything in an empty array
    return nil if nums.empty?
  
    probe_index = nums.length / 2 #=> 2, 1, 0
    case target <=> nums[probe_index]
    when -1
      # search in left
      bsearch(nums.take(probe_index), target) #=> 0
    when 0
    ...
    when 1
      ...
      sub_answer = bsearch(nums.drop(probe_index + 1), target) #=> 0
      sub_answer.nil? ? nil : (probe_index + 1) + sub_answer #=> 2 + 1 + 0 = 3
    end
  ...
  class Array
    ...
    def merge_sort(&prc)
      ...
      left = self.take(middle) #=> [2], [0]
      right = self.drop(middle) #=> [0, 1], [1]
      
      sorted_left = left.merge_sort(&prc) #=> [2], [0]
      sorted_right = right.merge_sort(&prc) #=> [0, 1], [1]
  
      merge(sorted_left, sorted_right, prc) #=> [0, 1, 2]
    end
